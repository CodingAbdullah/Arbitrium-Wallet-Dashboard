...
const ERC721TokenLookupsPage: FC = () => {
    ...
    const tokenHandler = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (address.current?.value.length === 42 && address.current.value.substring(0, 2) === '0x'){
            axios.post('https://18.221.208.44.nip.io/arb-erc721-token-lookup' , options)
            .then(response => {
            ...
            axios.post('https://18.221.208.44.nip.io/arb-erc721-token-transfers-lookup', options) // Get transfer data
            .then(response => {
