...
const MetricsNavbar = () => {
    ...
    useEffect(() => {       
        const fetchInfo = async () => {
            ...
            try {
                const response = await axios.get('https://18.221.208.44.nip.io/arb-price-lookup-information', options); // Retrieve price information right after render                
                    ...
                    <div>
                        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                            <li className="nav-item"> 
                                <p style={{ paddingLeft: '1rem', display: 'inline', color: 'white', marginTop: '1rem'}}>ARB Price: <b>{ priceInformation === undefined ? "Loading" : "$" + priceInformation.tokenPrice.usd.toFixed(2) }</b></p>
                            </li>
                            <li className="nav-item">
